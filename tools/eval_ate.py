...
def load_poses(path):
    ...
    with open(path, "r") as f:
        ...
        for line in f:
            ...
            c2w[:3, 1] *= 1   #! 涉及到初始坐标系的朝向问题,   *1 or *-1
            c2w[:3, 2] *= 1
            c2w = torch.from_numpy(c2w).float()
            poses.append(c2w)
    poses_num = len(poses)
    return poses, poses_num

if __name__ == '__main__':
    """
    ATE evaluation tool
    python ./tools/eval_ate.py --config ./configs/Replica/room0.yaml
    """
    ...
    parser.add_argument('--config', type = str, help = 'Path to config file',
                        default='./configs/Replica/room0.yaml')
    args = parser.parse_args()
    cfg = load_config(args.config)
    scale = cfg['scale']
    output_folder = cfg['data']['output_folder']
    input_folder = cfg['data']['input_folder']
    if cfg['dataset'] == 'tum' or cfg['dataset'] == 'scannet':
        estimate_c2w_list, estpose_num = load_poses(f'{output_folder}/trajectory_est.txt')
        gt_c2w_list, gtpose_num = load_poses(f'{output_folder}/gt_pose.txt')
    else:
        estimate_c2w_list, estpose_num = load_poses(f'{output_folder}/trajectory_est.txt')
        gt_c2w_list, gtpose_num = load_poses(f'{output_folder}/traj.txt')
    if estpose_num != gtpose_num:
        print('the num of gt_pose:{} is not equal to est_pose:{}!!!'.format(gtpose_num, estpose_num))
        sys.exit()
    poses_gt, mask = convert_poses(gt_c2w_list, gtpose_num, scale, gt=True)
    poses_est, _ = convert_poses(estimate_c2w_list, gtpose_num, scale, gt=False)
    poses_est = poses_est[mask]
    evaluate(poses_gt, poses_est,
                plot=f'{output_folder}/eval_ate_plot.png')
